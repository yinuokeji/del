...
echo 正在抓取IVI log
echo.
echo step 3:ctrl+C 停止IC log抓取
echo step 4:然后选择N导出log
adb -host shell "adb_ivi shell 'logctl -v threadtime > var/%n%.txt'"
echo.
adb -host shell "adb_ivi pull var/%n%.txt  var"
adb -host pull var/%n%.txt  .\\"%p%"
echo.
echo log抓取已结束，请查看
explorer ".\%p%"
pause